...
func (banco Db_user) PararDeSeguir(usuarioID, seguidorID uint64) error {
	statement, erro := banco.db.Prepare(queries.Q.PararDeSeguir)
	if erro != nil {
		return erro
	}
	defer statement.Close()
...
func (banco Db_user) BuscarSeguidores(usuarioID uint64) ([]modelos.Usuario, error) {
	linhas, erro := banco.db.Query(queries.Q.BuscarSeguidores, usuarioID)
	if erro != nil {
		return nil, erro
	}
	defer linhas.Close()
